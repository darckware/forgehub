...
export default function ProjectScopePage() {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-semibold">{t("page.title")}</h1><p className="text-sm text-muted-foreground">{t("page.description")}</p></div>
    {projectId && <Link className={buttonVariants({ variant: "outline", size: "sm", className: "gap-1.5" })} to={`/projects/${projectId}`}><GitPullRequestArrow className="h-4 w-4"/>{t("changeRequest.link")}</Link>}
    </div>
    ...
    {scope && <div className="grid gap-6 xl:grid-cols-[360px_1fr]"><Card><CardHeader><CardTitle className="text-base">{t("addItem.title")}</CardTitle><CardDescription>{t("addItem.description")}</CardDescription></CardHeader><CardContent><form noValidate className="space-y-3" onSubmit={async e=>{e.preventDefault();await add.mutateAsync({scopeId,...form,acceptance_criteria:form.criterion?[{criterion:form.criterion}]:[]});setForm({...form,system_element_id:"",criterion:""});}}>
