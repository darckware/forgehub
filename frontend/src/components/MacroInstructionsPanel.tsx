      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        {editing ? (
          <div className="space-y-4">
            ...
            <div>
              <p className="mb-1 text-xs text-muted-foreground">Uma instrução por linha, em linguagem natural (ex.: "clique em Update do Codex").</p>
              <textarea
                value={text}
            ...
            <div className="flex gap-2">
              <Button onClick={save} disabled={!name.trim() || lines.length === 0 || saving}>
                {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save macro
              </Button>
          ...
          <div className="space-y-3">
            <Button size="sm" onClick={() => setEditing("new")}><Plus className="mr-2 h-4 w-4" /> New macro</Button>
            {macros.isLoading && <p className="text-sm text-muted-foreground"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" />Loading macros…</p>}
            {!macros.isLoading && (macros.data?.length ?? 0) === 0 && (
              <p className="rounded-md border border-dashed p-6 text-center text-sm text-muted-foreground">No macros for this target.</p>
            )}
            {macros.data?.map((macro) => (
              <div key={macro.id} className="flex items-center gap-3 rounded-md border p-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-muted-foreground">{macro.description || `${macro.lines.length} instruction(s)`}</p>
                </div>
                <Button size="sm" onClick={() => sendToAthos(macro)}>
                  <Bot className="mr-1.5 h-3.5 w-3.5" /> Send <Send className="ml-1.5 h-3 w-3" />
                </Button>
