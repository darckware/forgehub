...
function TaskPackageRow({ projectId, wave, task, packages }: { projectId: string; wave: ExecutionWave; task: ProjectTask; packages: WorkPackage[] }) {
  ...
  return <div className="space-y-3 rounded-md border p-3">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><span className="font-medium">#{task.number} {task.title}</span> <Badge variant="outline">{task.status}</Badge></div>{latest && <span className="text-xs text-muted-foreground">Package r{latest.revision} · {latest.status}</span>}</div>
...
function WaveCard({ projectId, wave, tasks }: { projectId: string; wave: ExecutionWave; tasks: ProjectTask[] }) {
  ...
  return <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><div><CardTitle className="text-lg">{wave.name}</CardTitle><CardDescription>WIP {wave.wip_limit} · baseline {wave.baseline_id.slice(0, 8)}</CardDescription></div><Badge>{wave.status}</Badge></div></CardHeader><CardContent className="space-y-4">
    ...
    {links.length > 0 && <div className="space-y-1 text-xs">{links.map((item) => { const linkedTask = tasks.find((task) => task.id === item.task_id); return <div key={item.task_id} className={item.eligible ? "text-emerald-600" : "text-destructive"}>{linkedTask ? `#${linkedTask.number} ${linkedTask.title}` : item.task_id.slice(0, 8)} · {item.eligible ? "eligible" : item.reasons.join(", ")}</div>; })}</div>}
