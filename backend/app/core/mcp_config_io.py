"""Format-agnostic MCP server config file I/O -- the pure read/write engine
shared by `agent_mcp.py` (per-agent config, e.g. `<home>/config.yaml`) and
`project_mcp.py` (per-project `.mcp.json`). Extracted 2026-07-28 so a second
caller (project-scoped MCP) doesn't have to duplicate the YAML/TOML/JSON
surgical-write logic or reach into `agent_mcp.py`'s private functions.

Nothing here knows about agents, projects, or runtimes as domain concepts --
it only knows how to read/write one server entry inside one config file,
given a `McpConfigFormat` describing that file's shape. Callers own their
own host-path resolution policy (agent home vs. project working directory)
and pass in an already-resolved host path.
"""
...
class McpConfigFormat:
    """How one config file stores its MCP servers."""

    key: str
    filename: str
    # True when the file sits beside the given base directory rather than
    # inside it (e.g. Claude Code's per-agent ~/.claude.json). Project-scoped
    # formats never set this -- .mcp.json lives directly in the project root.
    ...
    # "disabled" is true-means-off, None means the format has no toggle.
...
def resolve_host_file(host_path: str | None) -> Path | None:
    ...
    does not exist anywhere (an unconfigured target, not an error)."""
    if not host_path:
        return None
...
def read_servers(host_path: str | None, fmt: McpConfigFormat) -> list[McpServerInfo]:
    """Every MCP server configured in this file, in file order. An absent
    file means "none configured yet", not an error -- most formats only grow
...
def _entry_dict(server: McpServerInfo, fmt: McpConfigFormat) -> dict:
    ...
    if fmt.toggle_field:
        # Only write the toggle when it says something: formats all default
        # to on, so an "enabled: true" line is noise in a hand-read config,
        # and "disabled: false" doubly so.
...
def write_server(
    ...
    config file at `host_path`, and return the host path actually written.

    The file is created when absent -- adding the first MCP server to a
    target that never had one is a normal first use, and every format has a
    valid empty document. The containing directory is not created: that would
    mean the target's directory doesn't exist, which is a registration error
    the operator should see rather than have papered over.
    """
