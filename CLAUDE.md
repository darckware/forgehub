...
**The Software Factory section is deliberately empty in the sidebar** (2026-07-27). Marcelo is designing the module's new concept from scratch, so the section shows no menu items at all rather than a menu assembled out of the screens that happen to exist. Every entry stays in `navSections.ts` with `hiddenInSidebar: true` — the routes (`/product`, `/projects`, `/conception`, `/system-map`, `/backlog`, `/tasks`, `/governance`, plus the already-hidden `/cockpit`, `/project-scope`, `/pipeline`, `/pipeline-templates`, `/governance/policies`) stay live and findable in Cmd/Ctrl+K, which is a search surface, not a menu. Deleting the entries would remove the screens, not empty the menu. `Sidebar.tsx` skips a section whose visible entries are all hidden, so the header disappears with them. **Do not add a new phase/stage taxonomy here without an approved spec under `docs/modules/`** — see the Definition Gate in `docs/README.md`. The previous five-phase organization was undone because it diverged from `docs/architecture/PLANNING_DELIVERY_ARCHITECTURE.md` and three of its five labels promised screens that don't exist.

The domain's own granularity is unchanged by that and is worth knowing before designing the new section:

- **Product** is durable. Conception and the System Map are recorded at product level (`product_concepts` and `system_blueprints` each hold a UNIQUE FK to `products`), so every project inherits them from its product.
- **Every evolution of a product becomes a Project**, which carries the description of what will be built. `project_scopes`, `planning_items` and `project_tasks` are all project-scoped.
- The full chain is **Product → Project → Planning (groups) → Task**.

The Cockpit (`/cockpit`) is grouped by that same tree and **carries the registration of both**: create a product, create a project under it, and link into each one's detail page. **Artifacts left the navigation** (2026-07-26): per-phase detail is written as `.md` files instead (Docs and Conception's Documentation step). **Kanboard left the sidebar** too, on the way to native task management — its route stays reachable and the client/sync/`kanboard_*` columns stay in place, because the external board is still written by other ecosystem agents.
