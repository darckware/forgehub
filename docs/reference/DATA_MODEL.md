...
Canonical artifact referenced by `docs/specs/SPEC.md` §12 ("Next Spec Artifacts" → DATA SPEC). This document is the data dictionary and entity-relationship map for every table ForgeHub owns in the `company` schema of the shared `company_postgres` instance (topology: `docs/reference/DB_README.md`).
...
- Business rules requiring a second statement or cross-row checks (existence of a referenced row, state-machine transitions) are enforced at the API route layer, not the DB — see `docs/reference/BUSINESS_RULES.md`.
...
No `CheckConstraint` exists yet on `pipeline_stages.status` / `project_pipelines.status` / `pipeline_stage_gates.status` — these remain free strings today; flagged as a possible follow-up hardening pass, out of scope for this round (see `docs/reference/BUSINESS_RULES.md` §7).
...
> **Superseded decision:** an earlier round of this session deferred adding fine-grained artifact types (screen/component/table/etc.), registering screen specs as `spec` and the tech-stack doc as `other`. Revisited and reversed later in the same session — see `docs/reference/BUSINESS_RULES.md` §8.
...
- `pipeline_templates` has zero seeded rows in any environment — there is no default "standard SDLC" template shipped with the app today; one is proposed as part of the ForgeHub pilot project (see `docs/reference/BUSINESS_RULES.md` and the Project entity created for ForgeHub itself in the live system).
- **Kanboard sync (removed 2026-07-28):** an earlier session added `POST /api/v1/tasks/{id}/sync-kanboard`, the `app/core/kanboard_client.py` JSON-RPC client, `project_tasks.kanboard_task_id`, and the frontend's `/kanboard` iframe page. Kanboard was discontinued as an integration; all of this code, the corresponding columns/constraints, and the `/kanboard` route were removed. Task management and inter-agent communication are now handled entirely through ForgeHub's own MCP server (`forgehub-messages`) and dispatched natively via the Messages channel (`AgentDemand`, `origin_type="task"`), surfaced in the app by the Messages component/screen — see `CLAUDE.md`'s "Every task is executed through the Messages channel" note.
