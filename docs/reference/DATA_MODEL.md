...
- **Kanboard sync (removed 2026-07-28):** an earlier session added `POST /api/v1/tasks/{id}/sync-kanboard`, the `app/core/kanboard_client.py` JSON-RPC client, `project_tasks.kanboard_task_id`, and the frontend's `/kanboard` iframe page. Kanboard was discontinued as an integration; all of this code, the corresponding columns/constraints, and the `/kanboard` route were removed. Task management and inter-agent communication are now handled entirely through ForgeHub's own MCP server (`forgehub`) and dispatched natively via the Messages channel (`AgentDemand`, `origin_type="task"`), surfaced in the app by the Messages component/screen — see `CLAUDE.md`'s "Every task is executed through the Messages channel" note.
