...
Canonical artifact for the **Contextualização** pipeline stage (`docs/reference/BUSINESS_RULES.md` §10). Captures the business context that precedes — and should be cited by — the PRD, rather than letting that context live only in conversation history.
...
- Stack is fixed per `docs/reference/TECHNOLOGY.md` (FastAPI/SQLAlchemy/Postgres backend, React/Vite/shadcn frontend) — not open for re-litigation per project.
...
- `is_locked` (Artifact, ProjectStructureNode) is advisory, not a filesystem permission — see `docs/reference/BUSINESS_RULES.md` §8.
