...
> **Nota de estado:** este guia descreve a experiência operacional desejada e contém automações ainda não disponíveis de ponta a ponta. Para distinguir comportamento implementado, parcial e proposto, e para consultar a classificação de projetos e a revisão atributo por atributo, use [Arquitetura de Planning e Delivery](../PLANNING_DELIVERY_ARCHITECTURE.md). Agentes CLI devem seguir também o [Protocolo de Desenvolvimento](../architecture/AGENT_CLI_DEVELOPMENT_PROTOCOL.md) e nunca presumir que um fluxo descrito aqui já está automatizado.
