...
- **Field-name mismatch between frontend `ProjectPlan` zod schema and backend `ProjectPlanOut`.** Frontend expects `start_date`, `target_date`, `is_baselined` (boolean) (`useProject.ts:27-37`); backend actually returns `estimated_start_date`, `estimated_end_date`, and `status` (string enum `draft|approved|baselined|superseded`, see `backend/app/db/models/project.py` and `docs/reference/DATA_MODEL.md` §3.2). Any future wiring of a real plan-fetch hook needs to fix this schema, not just add the missing endpoint call.
