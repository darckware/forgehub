...
| `frontend/src/pages/pipeline/PipelineForm.tsx` | Shared create/edit form (react-hook-form + Zod resolver): name, project select, template select (optional, create only — `showTemplate={false}` on edit), status select, is_active checkbox. |
...
- Submitting the form calls `createPipeline.mutate(...)`, sending `template_id: values.template_id || undefined` — collapses the empty-string default to `undefined` before POST; the backend instantiates the chosen template's stages on create. On success, closes the form.
...
**Create-from-template works as of 2026-07-15.** `create_pipeline` now validates `template_id` (404 on unknown) and instantiates the template's stages and their required artifacts as real `PipelineStage`/`PipelineStageRequiredArtifact` rows; `template_id` and explicit `stages` in the same payload are rejected with 422. The create form (`PipelineForm.tsx`) offers a template picker (Select fed by `usePipelineTemplates`) and the field name matches the backend (`template_id` — previously `pipeline_template_id`, silently dropped by pydantic, so template selection was a no-op). The edit flow hides the template picker (`showTemplate={false}`): PATCH ignores `template_id` by design.
- Template management itself lives on the Templates page (`/pipeline-templates`); there is still no stage/gate/required-artifact editing UI on this screen — `POST /pipelines/{id}/stages` etc. remain API-only.
...
- ~~`pipelineCreateSchema` declared `pipeline_template_id` while the backend expects `template_id`~~ — fixed 2026-07-15 (schema, form, and both mutate call sites now use `template_id`).
