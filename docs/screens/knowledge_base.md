...
- **Host bind mount**: `backend/app/api/routes/vault.py` reads `VAULT_ROOT = Path("/vault")` inside the container; `docker-compose.yml` mounts the complete `/root/.hermes/knowledge_base:/vault` root read-write. ForgeHub therefore exposes per-agent inboxes and `shared/` reviewed knowledge, while Obsidian may edit the same Markdown as a complementary client. If the host Knowledge Base root is unavailable, both `GET /tree` and `GET /graph` return 404 ("Vault is not mounted") and the screen shows the tree-load error state.
