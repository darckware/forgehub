...
### 6.6 Governança em camadas de execução — implementado 2026-08-17

Um incidente real (Athos narrou progresso concreto sobre um projeto — registro em board, arquivo "inicializado" — sem que nada disso existisse de fato; `dispatch_status="completed"` foi aceito sem nenhuma verificação) motivou implementar boa parte do que a §6.4 e o item **P0.10** do backlog (§8) já pediam: `Task done exige execução concluída/verificada, evidência e subtasks/dependências completas`. Plano completo, com toda a investigação que o precedeu, em `.claude/plans/resilient-twirling-blossom.md` (Porthus, sessão 2026-08-17).

**O que foi construído, no caminho canônico (`task.py`/Messages, não no Execution Wave/Work Package legado — ver nota de reconciliação abaixo):**

- `_dispatch_task_by_id` (`api/routes/task.py`) agora cria uma `TaskExecution` real por despacho, ligada ao `AgentDemand` via a nova coluna `task_execution_id` (migration `f3a7c1e9d284`).
- `_finalize_dispatch` (`api/routes/demand.py`) nunca mais escreve `TaskExecution.status="completed"`/`"verified"` a partir só do exit-status do host-bridge: um processo que terminou limpo vira `"reported"` — "o agente disse que terminou", não "verificado". O corpo da resposta pode declarar `EVIDENCE: <ref>` (`file:`/`git:`/`db:`), extraído por `_extract_evidence_ref`.
- `core/task_evidence.py` (novo, mesmo padrão de `core/task_health.py`, poll a cada 120s) verifica essa evidência contra a realidade: arquivo existe com `mtime` posterior ao início da execução (o cheque que teria pego o incidente original), commit existe no repositório do projeto, ou a entidade referenciada existe de fato no banco. Só então `"reported"` vira `"verified"` — ou `"failed"` com `Notification` se a checagem falhar.
- `ResponsibilityArea` (nova tabela, migration `a1d5e29b7c46`) resolve um dono padrão por `task_type`, com override por projeto — usada como fallback em `_dispatch_task_by_id` quando não há `target_agent_id` nem `TaskAssignment` ativa.
- `core/governed_approval.py` ganhou `request_task_approval`/`decide_task_approval`, reaproveitando o motor `ApprovalRequest`/`ApprovalDecisionRecord` já em produção para `product_concept` (decisão de 3 vias, `deny_self_approval` real — o agente que executou nunca pode ser quem aprova o próprio trabalho). Uma execução `"verified"` abre o pedido; `changes_requested` redespacha automaticamente com o feedback do revisor anexado (o "nova execução com feedback estruturado" que a §9 já descreve); `rejected` bloqueia a task; `approved` marca `done`. Deliberadamente **inerte por padrão** — só ativa quando existe um `PolicyBinding` ativo para `target_type="project_task"` (nenhum foi semeado automaticamente; ativação é decisão explícita, ver §8/P0 abaixo).
- `_ensure_subtasks_completed` fecha o outro lado do gate: uma task pai não conclui com subtask (`parent_task_id`) pendente — o fan-out em si (`create_task` com `parent_task_id`, `list_tasks?parent_task_id=`) já existia, só faltava esse gate.
- 27 testes novos (`test_task_evidence.py`, `test_responsibility_area.py`, `test_task_governed_approval.py`, mais 2 em `test_task.py`), incluindo a reprodução automatizada exata do incidente original. Suíte completa: 418/418.

**Reconciliação necessária com o Execution Wave/Work Package (`orchestration.py`) e `TaskExecutionReview`:** este documento (§8/9) e `ENGINEERING_LOOP.md` descrevem `ExecutionWave`/`LoopPolicy`/reviewer independente como a arquitetura-alvo — e `orchestration.py` já implementa boa parte disso, incluindo um mecanismo de revisão mais rico (`TaskExecutionReview`: score 0-100, revisor dispatchado de verdade via ForgeRouter com prompt estruturado `FORGEHUB_SCORE`/`FORGEHUB_DECISION`, `max_iterations` via `ProjectLoopPolicy`). **Só que esse pipeline é o que `CLAUDE.md` já registra como retirado para dispatch direto** (`POST /api/v1/orchestration/tasks/{id}/dispatch` → `410`, `ER-RUN-01`, "Messages stays the single executor") — o trabalho desta sessão foi construído sobre o caminho canônico atual (`task.py`), não sobre esse.

Isso deixa uma lacuna real, não fechada nesta sessão: as rotas de `TaskExecutionReview` (`POST/PATCH /api/v1/orchestration/executions/{id}/reviews`) **não foram retiradas junto com o dispatch** e continuam vivas — `update_execution_review` com `decision="approved"` marca `TaskExecution.status="verified"` e `ProjectTask.status="done"` diretamente, sem passar por `_ensure_evidence_verified`/`_ensure_subtasks_completed`/pela nova `ApprovalRequest`. Ou seja, existe hoje um segundo caminho que ainda pode fechar uma task ignorando os gates construídos em 2026-08-17. Risco prático baixo agora (`execution_waves`/`execution_work_packages`/`task_execution_reviews` têm **0 linhas reais** em produção — verificado nesta sessão), mas é uma lacuna de design, não uma garantia. Decisão pendente para quando o Work Package pipeline for retomado ou definitivamente descontinuado: unificar os dois mecanismos de revisão, ou fechar (410) as rotas de `TaskExecutionReview` junto com o dispatch que já foi retirado.
...
10. **Concluído no caminho canônico (2026-08-17, ver §6.6):** Task `done` exige execução verificada, evidência real e subtasks/dependências completas via `_ensure_evidence_verified`/`_ensure_subtasks_completed`. **Pendente:** as rotas `TaskExecutionReview` do Execution Wave/Work Package (`orchestration.py`) ainda contornam esse gate — não fechadas nesta sessão, ver reconciliação em §6.6.
